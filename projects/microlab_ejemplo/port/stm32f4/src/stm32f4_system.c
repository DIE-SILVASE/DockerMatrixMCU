...
{
  /* It can be done atomic by writing on register BSRR as well */
  if (value == true)
  {
    p_port->ODR |= BIT_POS_TO_MASK(pin); /* Turn it on */
  }
  else
  {
    p_port->ODR &= ~BIT_POS_TO_MASK(pin); /* Turn it off */
  }
}
...
{
  // TODO alumnos: Implement the function
...
void port_system_sleep()
{
  ...
  // port_system_power_stop();  // It only exits from stop mode with External Interrupts
